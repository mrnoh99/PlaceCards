...
enum Localization {
    static let englishTranslations: [String: String] = [
        ...
        "둘 다 비우면 좌표가 삭제됩니다. 하나만 채워지면 원래 값이 그대로 유지됩니다. \"주소로 좌표 확인\"은 AI 없이 Google Places로 위 주소를 좌표로 바꿔줍니다. \"저장하고 지도에서 확인\"은 지금까지 고친 것을 먼저 저장하고 지도 앱을 엽니다 — 거기서 장소를 공유하거나 화면을 찍어 보내면 새 카드를 만들지 않고 이 카드에 합쳐집니다. \"위치정보에 사진 GPS 사용\"을 켜두면, 위도·경도가 비어 있을 때 새로 추가하는 사진의 GPS를 좌표로 저장합니다(주소와 100m 이상 차이 나면 저장하지 않고 알려드립니다). 이미 채워져 있으면 새 사진이 그 위치에서 찍힌 게 맞는지만 확인합니다.":
            "Leaving both blank removes the coordinate. Filling in only one keeps the original value. \"Find Coordinates from Address\" turns the address above into coordinates via Google Places, no AI needed. \"Save and Check on a Map\" saves your edits first, then opens a map app — share the place back from there, or send a screenshot of it, and it is merged into this card instead of starting a new one. Turning on \"Use Photo GPS for Location Info\" saves a newly added photo's GPS as the coordinates when latitude/longitude are blank (unless it's more than 100m from the address, in which case you're told and nothing is saved). If they're already filled in, this only checks whether the new photo looks like it was actually taken there.",
        "저장하고 지도에서 확인": "Save and Check on a Map",
        ...
        "\"을 지울 수 없습니다": "\" can't be deleted",
        "게시판 \"": "Delete the board \"",
        "사용자가 취소했습니다.": "Cancelled by user.",
        "사진에서 고르기": "Choose from Photos",
        "장소가 들어 있는 게시판은 지울 수 없습니다. 장소는 여러 게시판에 들어갈 수 있어서, 게시판만 지우면 그 장소가 어디에도 속하지 않은 채 남습니다. 먼저 장소를 옮기거나 빼주세요.": "A board with places in it can't be deleted. A place can belong to several boards, so deleting the board alone would leave it belonging to none. Move or remove its places first.",
        "클립보드에 복사했습니다.": "Copied to the clipboard.",
        "\"파일로 공유\"는 사진까지 담은 폴더를 내보냅니다. \"텍스트로 복사\"는 사진 없이 목록만 복사합니다.": "\"Share as File\" exports a folder with the photos in it. \"Copy as Text\" copies the list only, without photos.",
        "사용자 보드": "My Boards",
        "사진": "Photos",
