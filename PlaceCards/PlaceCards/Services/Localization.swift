...
enum Localization {
    static let englishTranslations: [String: String] = [
        ...
        "\"을 삭제할까요?": "\"?",
        "\"의 좌표로 저장할까요?": "\" as its coordinates?",
        ...
        "둘 다 비우면 좌표가 삭제됩니다. 하나만 채워지면 원래 값이 그대로 유지됩니다. \"주소로 좌표 확인\"은 AI 없이 Google Places로 위 주소를 좌표로 바꿔줍니다 — 상세보기의 \"지도에서 열기\"로 정확한 주소를 먼저 확인한 뒤 여기 채우고 눌러보세요.": "Leaving both blank removes the coordinate. Filling in only one keeps the original value. \"Find Coordinates from Address\" turns the address above into coordinates via Google Places, no AI needed — first confirm the exact address using \"Open in Map\" in the detail view, then fill it in here and tap.",
        ...
        "사진 추가": "Add Photo",
        "사진에 위치 정보(GPS)가 있습니다. 이 위치를 \"": "The photo has location info (GPS). Save this location as \"",
