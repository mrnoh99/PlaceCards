...
enum Localization {
    static let englishTranslations: [String: String] = [
        ...
        "앱 화면 전체에서 사용하는 언어입니다. 사진 스캔 결과의 언어는 아래 \"AI 응답 언어\"에서 따로 정합니다.": "This is the language used throughout the app's screens. The language of photo-scan results is set separately below, under \"AI Response Language\".",
        ...
        "모든 게시판·장소를 직접 고른 파일로 백업하거나, 백업 파일에서 복원합니다 — 복원하면 지금 앱에 있는 모든 데이터가 그 파일 내용으로 교체됩니다. 사진도 함께 포함되므로 다른 기기에서 복원해도 정상적으로 보입니다(그만큼 파일 용량은 커질 수 있습니다).": "Back up every board and place to a file you choose, or restore from a backup file — restoring replaces everything currently in the app with that file's contents. Photos are included too, so they show up correctly even when restoring on a different device (the file will be larger as a result).",
        "모름": "Unsure",
        "무료": "Free",
        ...
        "사진 스캔으로 채워지는 카테고리·메모 같은 텍스트를 어떤 언어로 작성할지 정합니다. 앱 화면 자체의 언어(한국어)에는 영향을 주지 않습니다.": "Sets the language for text filled in by photo scanning, like category and memo. This doesn't affect the app's own screen language.",
        ...
        "아이콘": "Icon",
        "아직 공유 시도 기록이 없습니다. 사진 공유 시트에서 PinSpots를 선택하면 여기에 결과가 표시됩니다.": "No share attempts recorded yet. Choose PinSpots from a photo share sheet and the result will show up here.",
        "알 수 없는 오류": "Unknown error",
        "알림": "Notice",
        "여러 제공자의 키를 등록해두면, 아래 \"AI 제공자 우선순위\" 순서대로 시도하다가 하나가 실패(호출 한도 초과, 오류 등)해도 자동으로 다음 제공자로 넘어갑니다.": "Register more than one provider's key and, when one fails (rate limit, error, etc.), it automatically falls through to the next in the \"AI Provider Priority\" order below.",
        ...
        "기기에서 글자를 읽어 장소 이름을 찾았습니다. 나머지 정보는 Google 확인으로 채워집니다.": "Found a place name by reading the text on this device. The rest is filled in by verifying against Google.",
        ...
        "폴더": "Folder",
        "폴더 변경…": "Change Folder…",
        "폴더를 한 번 선택해두면, 앱을 열 때마다(위 주기당 최대 한 번) PinSpots가 그 폴더에 새 백업을 저장합니다.": "Once you choose a folder, PinSpots saves a new backup there each time you open the app (at most once per the interval above).",
