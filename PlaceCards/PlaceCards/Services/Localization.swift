...
enum Localization {
    static let englishTranslations: [String: String] = [
        ...
        "사진에서 AI로 찾기": "Find places in photos with AI",
        "스크린샷을 넣으면 AI가 장소명을 읽어옵니다. 이 기능만 AI API 키가 필요하며, 키는 이 기기의 키체인에만 저장됩니다.": "Drop in a screenshot and AI reads the place name off it. This is the only feature needing an AI API key, and the key is stored only in this device's keychain.",
        ...
        "\"지도에서 열기\"로 최근에 연 카드예요. 방금 공유한 사진을 이 카드에 추가합니다. (AI 제공자가 등록되어 있지 않아 정보는 자동으로 읽지 않습니다 — 설정에서 등록하면 이용할 수 있습니다.)": "This card was recently opened via \"Open in Map\". The photo you just shared will be added to it. (No AI provider is registered, so info won't be read automatically — register one in Settings to use this.)",
