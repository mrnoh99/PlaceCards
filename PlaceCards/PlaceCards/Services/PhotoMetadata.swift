...
enum PhotoVisitDates {
    ...
    /// Naver의 실제 등록 정보와 맞춰진 것).
    ///
    /// 확정 전 좌표는 **어림값**이다. 지도 앱이 공유해 준 링크에서 뽑았거나
    /// (`sharedCoordinates`), 주소 텍스트를 지오코딩했거나, 사진 GPS를 그대로
    /// 옮겨 놓은 것이다. 앞의 둘은 가게가 실제로 있는 자리가 아니라 지도
    /// 화면의 중심이거나 그 근처일 수 있다 — 수백 미터가 예사다. 반면 사진의
    /// GPS는 기기가 그 자리에서 잰 **실측**이다.
    ///
    /// 어림값으로 실측을 반증하면 앞뒤가 바뀐다. 실제로 그 탓에 "GPS로
    /// 촬영위치찾기로 지도를 열어 장소를 공유해 만든 카드는 날짜가 안
    /// 들어오는데, 구글에서 검색해 저장하면 그때서야 들어온다"가 됐다.
    /// 확정하는 순간 좌표가 진짜 값으로 바뀌면서 같은 사진이 200m 안으로
    /// 들어왔던 것이다.
    ///
