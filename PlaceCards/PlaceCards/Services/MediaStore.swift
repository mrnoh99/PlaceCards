...
/// 앞으로 고르는 사진만 됐을 것이다.
///
/// **대가, 그리고 그 대가가 도로 이득이 되는 길.** 라이브러리에서 골라 온
/// 사진은 라이브러리에 **한 장 더** 생긴다. 앱이 가진 것은 원본이 아니라
/// 제가 저장할 때 줄여 놓은 사본이므로(`MediaStore.saveImage`), 그 사본이
/// 새 사진으로 들어간다. 부르는 쪽이 이 사실을 사용자에게 먼저 알린다.
///
/// 그런데 사진 앱에는 **중복 항목**(앨범 → 유틸리티)이 있고, 거기서
/// **병합**하면 두 장이 하나로 합쳐지면서 화질 좋은 쪽이 남고 앨범 소속은
/// 양쪽 것을 다 물려받는다. 즉 **원본이 PinSpots 앨범에 남는다.**
///
/// 이것이 중요한 이유는, 맨 처음 바라던 "이 사진의 원본으로 가기"가 그
/// 경로로 실제로 이뤄지기 때문이다. 공개 API가 없어 포기했던 목표를 사진
/// 앱이 제 기능으로 채워 준다. 사본이 하나 생기는 것이 막다른 골목이
/// 아니라 원본으로 가는 길목인 셈이다.
///
/// (사용자가 실기기에서 확인해 준 동작이다. 이 앱이 부르는 API가 아니라
/// 사진 앱의 기능이므로 여기서 코드로 보장할 수 있는 것은 없고, 안내
/// 문구로만 가리켜 준다.)
