...
struct PlaceCard: Identifiable, Codable {
    ...
    /// 다른 앱이 공유해 준 정보로 만들어진 카드. 홈의 "가져오기"가 이
    /// 표시만 보고 모은다.
    ///
    /// `sources`의 `SourceType`으로는 가릴 수 없다. 같은 `googleMapShare`
    /// 라도 공유 시트로 들어온 것과 앱 안에서 링크를 붙여 넣어 만든 것이
    /// 구별되지 않기 때문이다. 어디로 들어왔는지는 만들 때만 알 수 있으므로
    /// 그때 찍어 둔다.
    var isImported: Bool?
