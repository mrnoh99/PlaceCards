...
### 2026-09-23 (215차) — "지도에서 열기"가 특정 못 하는 곳을 엉뚱한 곳으로 보내던 것 수정. 빌드 46 → 47
...
214차에서 추가한 `naverVerified`/`kakaoVerified` 플래그가 정확히 이
구분을 위해 있었는데 `MapOpeners.swift`가 전혀 안 쓰고 있었다. 이제
...
### 2026-09-22~23 (214차) — 기기 간 동기화, 카카오맵, Apple 장소확정. 빌드 30 → 46
`claude/lightroom-ui` 브랜치, PR #96~#111(전부 병합). 사용자 지시 "다른
계정에서 작업을 진행할 예정이니 그 동안 한 일을 요약정리 저장해 놓아라"에
따라 이 항목을 남긴다 — 상세한 "왜"는 각 PR 설명에 있고, 여기는 무엇이
바뀌었는지의 지도다.

#### 기기 간 동기화 (CloudKit) — #96~#100, #102
같은 계정의 다른 기기끼리 게시판·장소·사진을 주고받는다. 전부 **수동**
(설정과 갤러리 툴바의 "지금 맞추기" 단추를 눌러야 돈다 — 백그라운드
자동 동기화 없음).
- 순서: 연결 확인 → **먼저 받아 병합 → 그다음 올리기**(올리기가
  `savePolicy = .allKeys`로 무조건 덮어써서, 받기 전에 올리면 다른
  기기의 최신 수정을 지운다).
- 카드·게시판 둘 다 아주 지운 것에 **묘비**를 남긴다(`purgedCardIDs`,
  `purgedBoardIDs`, `UserDefaults`에 저장) — 안 그러면 클라우드에 남은
  사본이 다음 동기화에 되살아난다. 실제로 지우는 길 넷(휴지통 영구
  삭제·비우기·기한 만료 자동 정리·게시판 삭제)이 전부 이 구멍에
  걸려 있었다.
- 게시판 이름·차례도 옮겨 간다 — `Board`에 `updatedAt`·`sortIndex`(둘 다
  옵셔널)를 더했다. 차례가 예전엔 배열 자리 자체였는데, 그러면 기기
  밖으로 못 나간다.
- 사진은 카드와 **다른 존**(`PlaceCardsPhotos`)에 있어, 맞출 때마다 전부
  다시 받지 않고 이름(=파일명 UUID)으로 견줘 없는 쪽으로만 오간다.
- `CloudSyncService`(새 파일), 엔터틀먼트에 CloudKit 추가.

#### 카카오맵 공유 — #109, #111
`kakaoMapShare`가 처음으로 실제로 쓰였다(이전엔 "절대 안 쓴다"고 적혀
있었다). 카카오 **API는 여전히 안 쓴다**(로컬 API 약관이 검색 결과
저장을 금지) — 대신 `KakaoPlaceLinkResolver`가 공유 링크
(`kko.to/…` → `applink.map.kakao.com` → `place.map.kakao.com/{id}`)의
**공개 페이지 메타 태그**만 읽어 이름·주소·좌표·대표 사진 한 장을
가져온다. 이 공유가 성공하면 **그 자체가 장소확정**이다(검색-선택 흐름이
따로 없다 — 이미 그 장소의 공식 페이지이기 때문).

#### 장소확정에 Apple 지도 추가 — #107, #108, #111
`AppleLocalSearchService`(새 파일, `MKLocalSearch`)가 Google/Naver와
같은 "검색해서 고르는" 흐름으로 낀다. API 키가 필요 없어
`EditPlaceCardSheet.confirmPlace()`가 Google → Naver → Apple 순으로
시도하고 Apple은 항상 마지막 보루로 시도된다. `PlaceSearchResult
.isFromGooglePlaces: Bool`은 제공자가 셋이 되며 `provider:
PlaceSearchProvider` enum으로 교체했다. `PlaceCard`에 `appleVerified`·
`kakaoVerified`(둘 다 옵셔널) 추가, `isPlaceConfirmed`가 넷을 다 본다.

지도 앱 관련 메뉴(지도 화면 탭, "GPS로 촬영위치찾기", "지도에서
열기") 전부 **Apple·Google·Naver·(Kakao)·(Tmap)** 차례로 맞췄다 —
같은 다섯 앱을 고르는 자리가 여럿인데 차례가 다르면 손이 기억한 자리를
잘못 누른다.

#### 그 밖
- 사진을 **낱장으로** 사진 앱에 담을 수 있게 함(전체 화면 사진 뷰어의
  "…" 메뉴). 사진 앱 중복 항목 → 병합 기능으로 **원본이 PinSpots
  앨범에 남는다**는 길을 안내 문구에 넣음(사용자가 실기기에서 찾아냄).
- 사진 방문 날짜: 확정 전 좌표(어림값)로 사진의 실측 GPS를 반증하지
  않게 고침 — "GPS로 촬영위치찾기로 만든 카드는 구글 검색 전까지 날짜가
  안 들어온다"는 버그였다. 확정된 좌표만 기준으로 쓰고, 그 전엔 사진
  끼리 서로 견준다.
- `Tools/localization/check.py`에 **이스케이프 검사**를 추가함(`—`
  처럼 Swift가 모르는 이스케이프가 사전에 들어가 CI를 두 번 터뜨린
  뒤였다).

#### 아직 실기기 확인이 안 된 것
카드/게시판 묘비(지우고 두 기기에서 맞춘 뒤 한 번 더 맞춰도 안
돌아오는지), 게시판 **이름** 변경 동기화(차례는 확인됨), Apple/Kakao
장소확정, 사진 낱장 담기. B 기기에서 한 번 원인 불명의 "삭제됨이 비어
있습니다" 화면이 나왔는데 재현 안 됨 — 코드상 동기화가 그 화면으로
보낼 경로는 없음을 확인해 둠.
