...
### 2026-09-26 (271차) — 편집 중에 지도에 다녀온 것이 **새 카드가 되던** 것
사용자 요청: "저장후 edit mode에서 지도의 장소를 보내거나 사진을 보내면 통합하게
만들어라".

#### 편집 시트에는 지도 앱으로 나가는 문이 아예 없었다
"돌아온 공유를 이 카드에 합친다"는 길은 이미 있다 — `MapOpenContext`에 카드 id를
적어 두면 돌아온 링크는 `MapLinkImportSheet`(`.linkToCard`), 돌아온 사진은
`MapScreenshotImportSheet`(`.photoToCard`)로 가서 **그 카드에** 합쳐진다.

그런데 그 id를 적는 곳은 `PlaceCardDetailView`의 "지도에서 장소 확인"과
`MapOpenMenu`뿐이었다. **`EditPlaceCardSheet`에는 지도로 나가는 문이 없었다** —
좌표 섹션의 안내가 "상세보기의 '지도에서 열기'로 주소를 먼저 확인한 뒤 여기 채우고
눌러보세요"라고 딴 화면으로 보내고 있었다. 그러니 편집하다 지도에 다녀와 장소를
보내면 id가 없어 "보드를 고르고 **새 카드를 만드는**" 길로 갔다.

#### Added — `EditPlaceCardSheet`의 "저장하고 지도에서 확인"
좌표 섹션에 `MapOpenMenu`를 둔다. Apple·Google·Naver·Kakao·Tmap, 지도 화면 탭과
같은 차례다.

**저장이 먼저인 이유가 둘이다.** 둘 다 §2.2에서 나온다.

- 돌아온 공유가 **지금 뜬 시트를 닫는다.** 저장 안 하고 나가면 편집한 것이 사라진다.
  `AddPlaceCardView`는 사진을 맡겨 두는 쪽을 골랐지만(`recordMapOpen(photoDatas:)`),
  편집 시트는 필드가 수십 개라 맡겨 둘 모양이 아니다 — 저장이 답이다.
- `recordMapOpen(cardID:)`가 가리키는 것은 **저장된 카드**다. 안 저장하고 나가면
  돌아온 장소가 고치기 전 카드에 합쳐진다.

사용자가 "저장후"라고 한 것이 정확히 이 순서다.

#### `MapOpenMenu`에 `beforeOpen`을 더한다
편집 시트가 거기에 `save()`를 건다. **`label`보다 앞에 선언했다** — 기본값 있는
클로저를 `label` 뒤에 두면 기존 호출부 넷(`GalleryView`·`PlaceCardDetailView`·
`PlacesMapView`·`PlaceCardListRow`)의 라벨 없는 트레일링 클로저가 이쪽에 붙는다.
트레일링 클로저는 **선언상 마지막** 클로저를 찾는다 — CLAUDE.md §1의 표에 실제로
낸 사례로 적혀 있는 그것이다.

`Tmap`도 `beforeOpen`을 부른다. Tmap만 `recordMapOpen`을 빼는 것은 "돌아온 공유가
이 카드 것일 리 없다"는 판단이지 "저장하지 말라"가 아니다 — 어느 지도를 눌렀느냐에
따라 저장이 되거나 안 되면 그게 더 이상하다.

#### 지도에 넘기는 질의는 `card`가 아니다
`cardForMapQuery`를 둔다 — 이름·주소·좌표만 **지금 화면 값**으로 바꿔 둔 사본이다.
`card`를 그냥 넘기면 안 된다. 이름이나 주소를 방금 고쳐 놓고 누르는 것이 이 기능을
쓰는 이유인데, `card`는 시트를 열 때의 값이라 **고치기 전 이름으로** 지도를 열게
된다. id는 안 바뀌므로 `MapOpenContext`가 가리킬 카드는 같다.

#### Changed — 좌표 섹션 안내 문구
"상세보기의 '지도에서 열기'로 먼저 확인하고 오라"는 딴 화면 심부름을 지웠다. 이제
그 자리에서 되고, 돌아온 것이 새 카드가 아니라 이 카드에 합쳐진다고 적는다.

#### 검증
괄호 균형 — `EditPlaceCardSheet`·`Localization` 0, `MapOpeners`는 HEAD와 **같음**
(그 파일의 `(` 차이 10은 주석 안 URL 때문이고 이 변경 전에도 같았다),
`Tools/localization/check.py` 통과(611항목 — 새 문구 하나, 고친 문구 하나),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 63 → 64.

`00_UI개편_기초.md` §2.2에 이 문(門)과 저장 순서를 적었다 — 그 절이 "지도 앱으로
나가는 모든 버튼은 예외 없이 문맥을 기록해야 한다"고 못 박은 자리다.
...
#### 지우면 이 기기가 제 백업 이름을 잊는다
백업 폴더 이름은 `placecards_auto_backup_<deviceID>`이고, 그 `deviceID`는
`UserDefaults`에 있다. **앱을 지우면 같이 날아간다.** 재설치본은 새 id를 매기므로,
목록에 없어도 붙여 보는 "아는 이름 셋"(이 기기의 폴더·옛 파일·공용 파일)에 **이
기기의 옛 백업조차 들어 있지 않다.**

그러면 남는 길은 컨테이너 목록뿐인데, **콜드 런치 직후에는 그게 비어 있을 수
있다.** `contentsOfDirectory`는 iCloud가 그 폴더를 훑어 자리를 잡아 둔 뒤에야 다른
기기의 백업을 보여 준다. 재설치가 정확히 그 순간이다 — 그래서 한 번 보고 없으면
없는 것으로 끝났다.

#### Fixed — 못 찾았으면 뒤에서 더 본다
처음 한 번은 그대로 기다린다(대개 그때 온다). **못 찾았을 때만** 3초 간격으로 최대
열 번 다시 본다. 자료가 들어오는 순간 멈춘다 — 그 사이 사용자가 손으로 "지금
맞추기"를 눌렀을 수도 있고, 그때 또 합칠 이유가 없다.

**화면은 붙잡지 않는다.** 시작 화면은 첫 시도가 끝나면 바로 내려간다. 자료가 늦게
도착해도 `@Published`가 화면을 다시 그리므로 보이는 데 문제가 없고, 백업이 정말
없는 새 사용자는 한 순간도 더 기다리지 않는다.

`@MainActor`를 명시했다 — 그래야 안의 `Task { }`가 같은 액터를 물려받아
`storageService`(`@MainActor`)와 `@State`를 건드리는 것이 액터를 넘지 않는다.

#### 참고 — "지금 맞추기"는 이것과 다른 길이다
`CloudSyncService.syncNow`는 CloudKit 레코드를 직접 주고받고, **변경 토큰을 저장하지
않는다.** 누를 때마다 전량 받기이므로 `deviceID`와도 무관하다. 재설치한 기기에
자료를 되돌리는 가장 확실한 길은 여전히 이쪽이다.

#### 남은 것 — 실행 직후 크래시
원인 미상이다. 빌드 57–60에 들어간 변경에는 강제 언래핑·인덱스·`try!` 같은 트랩
낼 수 있는 코드가 한 줄도 없고, `mediaDidChange`도 전부 `@MainActor` 안에서 불린다.
지우고 다시 설치하니 됐다는 것은 **디스크에 있던 자료나 `UserDefaults`가 방아쇠**였음을
가리키지만, `Exception Type` 없이는 어느 쪽인지 가릴 수 없다. 분석 데이터가 오면
그때 잡는다.

#### 검증
괄호 균형 1파일 0, `Tools/localization/check.py` 통과(610항목),
`Tools/ocr-regression/check.py` 통과(50건). 새 번역 문자열 없음. 빌드 60 → 61.

### 2026-09-23 (267차) — 안 내려온 iCloud 폴더를 고르면 **마냥 서 있던** 것
사용자 신고: "가져올 게시판 파일을 다운로드 못하고 마냥있는다. 폴더에 가서
다운로드하고 가져오면 쉽게 정확히 가져온다." (266차에서 표지 사진은 해결됨을 확인.)

#### 시간 제한이 없는 자리가 하나 있었다
`stageBundle`이 고른 폴더를 `FileManager.copyItem`으로 **디렉터리째** 복사했다
(266차에 들어간 코드다). 그 안에 아직 안 내려온 항목이 하나라도 있으면 파일
제공자가 줄 때까지 **시간 제한 없이** 붙잡힌다. 파일 앱에서 먼저 받아 두면 잘
되던 것이 그래서다.

이제 **한 장씩** 옮긴다 — `metadata.json` 하나, 그리고 `photos/` 안에서 점으로
시작하지 않는 이름만(`writePhotos`와 같은 규칙: 안 내려온 파일은 `.<이름>.icloud`
플레이스홀더로 목록에 나오고, 그 껍데기를 옮기면 사진인 양 `MediaStore`까지 간다).
설령 뭐가 남아 있어도 그 한 장이 빠질 뿐 서지는 않는다.

#### 시간이 다해도 **아무 말이 없었다**
`ensureDownloaded`는 아무것도 안 돌려줬다. 그래서 부르는 쪽은 다 받은 것과 시간이
다한 것을 구별할 수 없었고, 그대로 밀고 나갔다 — 위의 블록이거나, 사진이 반쯤
빠진 게시판이었다.

`downloadIfNeeded`가 **끝내 안 온 개수**를, `ensureDownloaded`가 **다 왔는지**를
돌려준다. 가져오기와 복원 두 곳이 그 답을 보고, 안 왔으면 사용자가 이미 찾아낸
그 길을 그대로 알려 준다 — "파일 앱에서 이 폴더를 받아 둔 뒤 다시 골라주세요."

#### 기다리는 시간을 60초에서 180초로
사진 300장짜리 게시판은 60초 안에 다 오지 못한다. 예전에는 그래도 조용히
지나갔으니 짧아도 티가 안 났지만, 이제 시간이 다하면 안 왔다고 **말하므로**
말하기 전에 충분히 기다려 주는 편이 낫다. 그동안 화면에는 진행 표시가 떠 있다.

#### 남은 것
이 변경은 "서 있는" 것을 없애고 안 왔으면 안 왔다고 말하게 한다. 그래도
**내려받기 자체가 안 되는** 것이라면 — `startDownloadingUbiquitousItem`은 파일
제공자가 돌려준 URL에는 안 먹을 수 있다 — 이제 그 사실이 메시지로 드러난다.
그때는 `NSFileCoordinator`로 조정된 읽기를 걸어 제공자가 실체화하게 하는 것이
다음 수다.

#### 검증
괄호 균형 4파일 0, `Tools/localization/check.py` 통과(610항목 — 새 문구 하나),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 59 → 60.

### 2026-09-23 (266차) — 가져오기가 보안 스코프를 다시 잡던 것, 그리고 표지 사진
사용자 신고: "보드가져오기때 보드아이콘 사진이 안온다". 보낸 쪽도 받는 쪽도 빌드 57.

#### 신고된 증상의 원인은 **꾸러미가 빌드 56 이전 것**이었다
같이 확인한 사실이 답을 갈랐다 — **카드 사진은 왔다.** `writePhotos`는
`photos/` 안의 파일을 **가리지 않고 전부** 옮기므로, 카드 사진이 왔다면 폴더는
성하게 건너온 것이다. 그러면 남는 답은 하나뿐이다: **표지 사진이 애초에 꾸러미에
없었다.**

`Board.coverPhotoPath`는 **빌드 57에서 생겼다**(264차). 빌드 56 이전에 내보낸
꾸러미에는 그 필드도, `photos/` 안의 표지 파일도 없다. 카드 사진은 그때도 실렸으므로
"사진은 오는데 게시판 표지만 안 온다"로 정확히 보인다.

즉 **앱이 아니라 꾸러미가 낡았다.** 새로 내보내면 표지가 실린다.

#### Fixed — 그래도 조용히 넘어가지는 않게
표지 경로만 남기고 파일이 없으면, 이 기기 어디에도 없는 파일을 영영 가리키는
게시판이 하나 생긴다. 게다가 가져오기는 id를 새로 매기므로 **원본이 고쳐 줄 길이
없고**, 그 경로가 동기화로 다른 기기까지 간다.

`importBoard`가 표지 파일이 실제로 왔는지 보고, 없으면 `coverPhotoPath`를 비운다.
표지는 기호로 돌아간다(`coverIcon`은 사진이 있어도 안 지우므로 늘 있다 — 264차).
사진은 게시판·카드를 넣기 **전에** 전부 디스크에 오므로 이 시점의 "있나 없나"는
확정이다.

#### Fixed — 보안 스코프를 **함수 하나 건너 다시 잡던 것**
위 증상과는 별개로, 261차에서 사진 복사를 `performImport`로 옮기면서 생긴 불안정을
없앤다. 고를 때 한 번 잡아 메타데이터만 읽고, 사용자가 "가져오기"를 누르면 그 URL로
스코프를 **다시** 잡아 사진을 복사하는 모양이었다. 그 재획득이 실패하면
`contentsOfDirectory`가 nil을 돌려주고 사진이 **한 장도 안 복사된 채 조용히** 끝난다 —
카드와 게시판은 이미 디코딩해 둔 메타데이터라 멀쩡히 들어오므로 실패가 보이지 않는다.

`BackupService.stageBundle(at:)`을 두어, **고르는 그 순간 스코프가 확실히 열려 있는
동안** 폴더를 통째로 임시 폴더에 복사한다. 그 뒤로는 우리 임시 폴더라 스코프가 필요
없다. `stageLegacyBackup`과 같은 모양을 돌려주므로 부르는 쪽은 둘을 구별하지 않고,
다 쓰면 똑같이 `discardStagedBundle(at:)`로 치운다.

#### Fixed — 가져오기가 `mediaDidChange()`를 안 불렀다
`restore(bundleAt:)`은 사진을 쓴 뒤 알려 주는데(265차) `importBoard`만 빠져 있었다.
이쪽은 사진이 **먼저** 오므로 가져오기 화면 자체는 멀쩡했지만, 이미 그려져 있던
갤러리·휴지통은 모르고 지나간다.

#### 검증
괄호 균형 2파일 0, `Tools/localization/check.py` 통과(609항목),
`Tools/ocr-regression/check.py` 통과(50건). 새 번역 문자열 없음. 빌드 58 → 59.
...
#### Added — `Board.coverPhotoPath`
**옵셔널이다** — 이 필드가 생기기 전에 저장된 게시판에는 없어서 nil로 디코딩된다
(§4). 사진이 있으면 사진이 표지고, 없으면 예전대로 `coverIcon`을 그린다.

**`coverIcon`은 사진이 있어도 안 지운다.** 메뉴 줄·칩·배지는
`Label(_:systemImage:)`이라 기호가 늘 있어야 하고, 사진을 뺐을 때 돌아갈 자리도
그것이다.

#### Added — `BoardCoverPicker`
새 게시판과 게시판 수정이 **같은 것**을 쓴다. 지금 표지 미리보기, "사진에서
고르기"(`PhotosPicker`), "사진 빼기", 그리고 기호 격자.

기호 격자는 사진이 걸려 있어도 누를 수 있고, 누르면 사진을 뺀다 — 기호를 눌렀는데
아무 일도 안 일어나는 것이 더 이상하다.

`MediaStore.saveImage(data:)`가 아니라 `saveImage(_:)`를 쓴다. 그쪽이 줄여
저장한다 — 표지는 48pt 칸에 그려지고, **이 사진은 백업과 기기 간 동기화에도
실린다.**

#### Changed — 표지 사진이 카드 사진과 같은 폴더에 산다
그래서 사진을 세는 **네 곳이 전부** 이것도 세야 한다. 하나라도 빠지면 조용히
깨진다:

| 자리 | 안 세면 |
|---|---|
| `BackupService.referencedPhotoNames` | 백업에 안 실려 복원하면 표지가 빈 칸 |
| `CloudSyncService.syncPhotos` | 다른 기기에서 표지가 빈 칸 |
| `StorageService.deleteMediaIfUnreferenced` | 카드를 지우다 표지 사진을 같이 지움 |
| `saveBoard` / `deleteBoard` | 갈아치운·지운 표지가 영영 남음 |

`deleteBoard`는 **게시판을 배열에서 뺀 뒤에** 표지를 정리한다 — 안 그러면 제
표지가 제 참조로 잡혀 영영 안 지워진다. `saveBoard`도 같은 이유로 배열을 고친
뒤에 부른다.

지우는 쪽은 전부 `deleteMediaIfUnreferenced`를 지난다(261차). 카드가 쓰는 사진을
표지로 골랐거나 두 게시판이 같은 것을 쓰면 안 지워진다.

#### 검증
괄호 균형 9파일 0, `Tools/localization/check.py` 통과(609항목, 새 문구 둘),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 56 → 57.

### 2026-09-23 (263차) — 가져온 게시판 이름이 겹치면 "(2)"를 붙인다
사용자 요청: "가져올때 보드가 이름이 같으면 이름에 (2) 를 붙이도록 해라".

아이폰에서 내보내 아이패드로 가져오면 **같은 이름의 게시판이 둘** 생긴다. id는
새로 매기므로 분명히 다른 게시판인데 목록에서는 똑같이 보여, 어느 쪽이 방금
가져온 것인지 알 수 없었다.

`BackupService.uniqueBoardName(_:existing:)`이 겹치면 `"이름 (2)"`, 그것도 있으면
`"이름 (3)"`을 만든다.

**목록을 그때그때 다시 본다.** 한 번에 여러 게시판을 가져올 때 방금 추가한 것과도
겹치면 안 되는데, `saveBoard`가 바로 `storageService.boards`에 넣으므로 매번
현재 목록을 읽는 것만으로 맞는다.

이미 `(2)`로 끝나는 이름을 벗겨 내지는 않는다. `"제주 (2)"`를 두 번 가져오면
`"제주 (2) (2)"`가 된다 — 보기에 좋지는 않지만, 사용자가 일부러 `(2)`로 끝나게
지은 이름을 건드리는 것보다 낫다.

Google Takeout 가져오기도 같은 길(`importBoard`)을 지나므로 함께 적용된다.

#### 검증
여기서 Swift를 못 돌리므로 **로직을 그대로 옮겨** 여섯 경우로 확인했다 —
겹침 없음 / 한 번 겹침 / 두 번 / 세 번 / 이미 `(2)`로 끝나는 이름 / 다른 이름들만
있는 경우. 한 번에 셋을 가져오는 경우도 `(2)·(3)·(4)`로 갈라지는 것을 확인했다.

괄호 균형 0, `Tools/localization/check.py` 통과(607항목),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 55 → 56.

### 2026-09-23 (262차) — "빈 게시판만 지운다" 규칙을 되살린다 (259차 정정)
사용자 지적:

> "가져온 폴더를 지워도 사진은 없어지지 않는다. 당연한 일이다. **이유는 여러
> 보드에 있을 수 있으니까**이다. 그러므로 폴더를 지우려면 그 안에 카드가 없어야
> 한다는 룰이 있었는데 그것이 무시되어서 일어난 일이다."

**259차가 그 규칙을 없앤 것이 틀렸다.** 당시 근거는 "`deleteBoard`가 214차부터
카드를 데려가지 않으니 조건이 필요 없다"였다. 카드를 안 데려가는 것은 맞지만
**그게 근거가 아니었다** — 규칙이 있던 이유는 **카드가 여러 보드에 들어갈 수
있다**는 것이고, 그래서 보드만 지우면 카드가 **어디에도 속하지 않은 채 남는다.**
먼저 비우게 해야 한다.

#### 규칙은 되살리되 단추를 숨기지 않는다
예전에는 조건을 `swipeActions` 묶음 **안**에 두어, 카드가 있는 보드에서는 그
묶음이 통째로 비었다. 내용이 빈 `swipeActions`는 아무것도 안 내놓는 대신 그
밀기가 줄 선택으로 떨어진다 — 259차가 고친 "왼쪽으로 밀면 갤러리로 넘어간다"가
그것이다. 숨기는 방식으로 되돌리면 그 버그가 같이 돌아오고, 사용자는 단추가 왜
안 나오는지도 알 수 없다.

그래서 **단추는 늘 내놓고, 누르면 왜 못 지우는지 말한다:**

> 장소가 들어 있는 게시판은 지울 수 없습니다. 장소는 여러 게시판에 들어갈 수
> 있어서, 게시판만 지우면 그 장소가 어디에도 속하지 않은 채 남습니다. 먼저
> 장소를 옮기거나 빼주세요.

비어 있으면 예전대로 삭제를 묻는다. 259차가 넣었던 "이 게시판만 없어집니다.
장소는 '모든 카드'에 그대로 남습니다."는 지웠다 — 이제 빈 게시판만 지우므로
할 말이 아니다.

#### 검증
괄호 균형 2파일 0, `Tools/localization/check.py` 통과(607항목),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 54 → 55.

### 2026-09-23 (261차) — 가져온 카드를 지우면 **원본 사진이 사라지던 것**
사용자 신고 둘. 뒤의 것이 실제 **데이터 손실**이다.

> "아이폰에서 보드를 내보내기 하고 아이패드에서 가져오기 하면 같은 이름의 보드가
> 생기는데 그 보드를 지워도 가져오기에 카드 전체가 남아있다. … 그런데 가져오기에
> 남아있는 카드를 삭제하면 **이미 있던 카드의 사진이 없어진다**."

#### ① 사진이 사라진 이유 — 두 카드가 **같은 파일**을 가리킨다
`importBoard`는 카드 id만 새로 매기고 **사진 이름(`MediaItem.localPath`)은 그대로
둔다.** 사진이 그 이름으로 저장돼 있어야 카드가 찾기 때문이다. 같은 기기에 원본이
이미 있으면 **가져온 카드와 원본이 같은 파일을 가리킨다.**

그 상태에서 `purge`가 사진 파일을 **조건 없이** 지웠다. 가져온 쪽을 지우면 원본의
사진이 같이 사라진다. 사진 낱장 삭제(`PlaceCardDetailView.deletePhoto`)도 같았다.

`StorageService.deleteMediaIfUnreferenced(_:excluding:)`를 두고, **다른 카드가 아직
가리키는 파일은 안 지운다.** 삭제됨(휴지통)에 있는 카드도 센다 — 되돌릴 수 있는
카드가 가리키는 사진을 지우면 되돌렸을 때 빈 카드가 된다.

`deletePhoto`는 순서가 중요해졌다. **`save` 뒤에** 부른다 — 그래야 이 카드가 더
이상 그 사진을 가리키지 않는 상태로 세어진다.

#### ② 게시판째 가져온 카드가 "가져오기"에도 들어가던 것
226차가 "바깥에서 들어온 것은 전부 가져오기에 모인다"를 지키려고 `isImported`를
찍고 있었다. 사용자 지적이 맞다 — **이 길로 들어온 카드는 이미 게시판이 정해져
있다.** 방금 그 게시판째 가져왔기 때문이다. 가져오기의 존재 이유가 "아직 갈 곳을
안 정한 카드를 모아 둔다"이므로 여기서는 할 일이 없고, 그래서 게시판을 지워도
카드가 가져오기에 남았다.

`importBoard`에서 그 표시를 없앴다. 게시판이 없는 카드는 위에서 이미 걸러지므로
(`guard !mapped.isEmpty else { continue }`) 여기까지 온 카드는 반드시 게시판이 있다.

이제 `isImported`를 찍는 곳은 `PlaceCardViewModel` 둘뿐이다 — 공유로 들어온 것과
"가져오기"를 보는 중에 "+"로 만든 것.

**이미 그렇게 찍힌 카드는 그대로다.** 어느 것이 게시판째 들어온 것인지 뒤늦게
가릴 방법이 없다. 가져오기에서 빼거나 게시판으로 보내면 없어진다.

#### 검증
괄호 균형 3파일 0, `Tools/localization/check.py` 통과(606항목),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 53 → 54.

### 2026-09-23 (260차) — iCloud에서 안 내려온 파일을 고르면 **멈춰 서던 것**
사용자 신고: "보드가져오기로 icloud 폴더에 download 안된 파일을 선택한 경우
진행이 안된다" → 정정: **"조용히 실패가 아니라 진행이 안되고 서있는다."**

#### 왜 서는가
파일 선택기는 **아직 안 내려온 iCloud 파일도 고르게 해 준다.** 목록에는
보이지만 이 기기에는 플레이스홀더만 있다. 그걸 `Data(contentsOf:)`로 바로
읽으면 Foundation이 파일을 받아오려고 **블록한다** — 실패로 떨어지는 게
아니라 그 자리에 멎는다. 화면에 진행 표시도 없어서 선 것처럼 보인다.

`CloudBackupService`는 이미 내려받기를 걸고 있었다(235차). **사용자가 파일
선택기로 고르는 길에만 그게 없었다.**

#### Added — `BackupService.ensureDownloaded(at:)`
읽기 전에 내려받기를 걸고 기다린다.

- **파일**이면 그 파일 하나.
- **폴더 형식**이면 `metadata.json`을 먼저 받아 읽고, **거기 적힌 이름으로**
...
기다리는 시간은 파일/메타데이터 20초, 사진 60초. **시간이 지나도 안 오면
멎지 않고 말한다**: "아직 iCloud에서 내려받지 못했습니다. 파일 앱에서 받아
둔 뒤 다시 골라주세요."

#### Added — 읽는 동안 진행 표시
가져오기 시트가 "파일을 읽는 중…"을 띄운다. 내려받는 데 한참 걸릴 수 있고,
**그동안 화면에 아무 표시가 없던 것이 "서 있다"로 보인 절반**이다. 설정의
복원도 같은 문구를 쓴다.

#### Changed — 내려받기 코드를 한 벌로
`CloudBackupService`의 `downloadIfNeeded`와 대기 시간 상수 둘을
`BackupService`로 옮겼다. 사용자가 고른 파일과 iCloud 컨테이너의 파일이 같은
사정이라 두 벌로 둘 이유가 없다.

#### 검증
괄호 균형 5파일 0, `Tools/localization/check.py` 통과(606항목, 새 문구 셋),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 52 → 53.

### 2026-09-23 (259차) — 아이폰에서 게시판 스와이프가 어긋나던 것
사용자 신고: "게시판 스와이프는 ipad 에서 iphone에서는 없다" → 자세히:
**"오른쪽으로 밀면 edit 왼쪽으로 밀면 갤러리로 넘어간다."**

증상 둘이 원인도 둘이었다.

#### ① 왼쪽으로 밀면 갤러리로 넘어간다 — **빈 `swipeActions`**
삭제 묶음이 `if storageService.placeCards(inBoard:).isEmpty` 안에 통째로
들어 있었다. **카드가 있는 보드에서는 그 묶음이 비고**, 내용이 빈
`swipeActions`는 아무것도 안 내놓는 대신 그 밀기가 **줄 선택으로 떨어진다** —
그래서 갤러리로 넘어갔다.

**그 조건 자체가 이제 근거가 없다.** 주석은 "스와이프가 장소 카드를 조용히
데려가지 못하게"라고 적고 있었는데, `deleteBoard`는 **214차부터 카드를 데려가지
않는다** — 그 보드에서 빼기만 하고 카드는 "모든 카드"에 남는다. 조건을 없애
삭제를 늘 내놓는다.

확인 문구도 고쳤다. "비어있는 게시판 …을 삭제할까요?" → "게시판 …을
삭제할까요?"이고, **"이 게시판만 없어집니다. 장소는 '모든 카드'에 그대로
남습니다."**를 덧붙였다 — 그 자리에서 가장 궁금한 것이 그것이다.

#### ② 오른쪽으로 밀면 수정만 — **`Menu`를 `swipeActions`에 넣었다**
`ExportBoardMenu`는 `Menu`였고 `.swipeActions(edge: .leading)` 안에 있었다.
그 자리는 `Button`을 기대하는 곳이라 **아이폰에서는 아예 렌더되지 않았다.**
아이패드에서는 나왔으므로 기기별로 갈렸다.

`ExportBoardSheet`로 옮기고, 행을 **길게 눌러** 나오는 컨텍스트 메뉴(수정 ·
내보내기 · 삭제)에서 연다. 컨텍스트 메뉴는 두 기기에서 같게 동작하고
`ShareLink`도 제대로 담는다.

**준비 시점도 제자리를 찾았다.** 예전에는 메뉴가 뜨기만 하면 `.task`가 돌아
**행을 스와이프해 여는 것만으로** 그 게시판의 사진이 전부 복사됐다(257차에
폴더 형식으로 바꾸기 전에는 그게 곧 메모리 폭발이었다). 이제는 사용자가
내보내기를 고른 뒤에만 돈다.

#### 새 문구 셋
"게시판 \"", "이 게시판만 없어집니다…", "클립보드에 복사했습니다.",
그리고 내보내기 시트의 설명 한 줄. "비어있는 게시판 \"" 키는 지웠다.

#### 검증
괄호 균형 2파일 0, `Tools/localization/check.py` 통과(603항목),
`Tools/ocr-regression/check.py` 통과(50건). 빌드 51 → 52.

### 2026-09-23 (258차) — 옛 백업을 **읽을 때** 죽던 것 (4단계 — 마지막 구멍)
사용자 신고: "가져오기하면 파일을 가져오면 crash". 결정적 단서는 그 다음 줄이었다 —
**"내보내기를 하면 json 파일이 만들어 지고 그 파일을 가져왔다."**

내보내기가 `.json`을 낸다는 것은 그 기기가 **257차 이전 빌드**라는 뜻이다(빌드 50부터
폴더). 즉 옛 형식 파일이고, 그것을 **읽는** 길은 255~257차가 건드리지 않았다.

#### 쓰는 쪽만 고쳤고 읽는 쪽이 남아 있었다
`ImportBoardSheet.handleFilePicked`가 `Data(contentsOf:)`와 `BackupService.decode`를
**메인 액터에서 그대로** 돌고 있었다. 옛 형식은 사진이 base64로 박혀 있어 파일
바이트(원본의 4/3)와 디코딩된 사진 사전이 동시에 메모리에 있다 — 쓸 때와 같은
크기가 읽을 때도 든다.

게다가 **미리보기가 사진 전체를 쥔 채 화면에 떠 있었다.** 사용자가 "가져오기"를
누를 때까지 수백 MB가 그대로 잡혀 있었다.

#### Added — `stageLegacyBackup(at:)`
옛 단일 파일을 **임시 폴더 형식으로 옮겨 놓고 메타데이터만 돌려준다.**

- 읽기·디코딩이 전부 **메인 액터 밖**에서 일어난다.
- 디코딩하자마자 사진을 임시 폴더로 내려놓고 `mediaFiles`를 떼어 낸다. 화면이
  드는 것은 메타데이터뿐이다.
- 돌려준 폴더는 폴더 형식 백업과 똑같은 모양이라, **그 뒤로는 호출부가 둘을 구별할
  필요가 없다.** 다 쓰면 `discardStagedBundle(at:)`로 치운다 — 안 치우면 사진
  수백 장이 임시 디렉터리에 남는다.

`JSONDecoder`가 통째로 주는 이상 디코딩 순간의 최고점 자체는 피할 수 없다. 피할 수
있는 둘(메인 액터, 그리고 계속 쥐고 있는 것)을 없앴다.

#### Changed — 옛 파일을 읽는 세 자리가 전부 이 길로
- `ImportBoardSheet` — 게시판 가져오기. **크래시가 난 그 자리다.**
- `SettingsView.performRestore` — 백업에서 복원.
- `CloudBackupService.restoreLegacyFile` — 콜드 런치의 iCloud 옛 파일.

#### Removed
`restore(from:)` · `restore(_:)`. 둘 다 사진이 박힌 payload를 통째로 받아
디코딩된 사진 사전을 손에 쥔 채 돌았다 — **읽는 쪽에 남아 있던 마지막 메모리
구멍이다.** 위 셋을 옮기고 나니 호출부가 0이 됐다.

`writeMediaFiles`는 남긴다. 옛 백업의 JSON을 **텍스트로 붙여넣는** 길이 아직
있고, 거기에는 사진이 박혀 있을 수 있다.

#### 검증
괄호 균형 4파일 0, `Tools/localization/check.py` 통과(600항목),
`Tools/ocr-regression/check.py` 통과(50건), 죽은 함수 호출 0. 빌드 50 → 51.
