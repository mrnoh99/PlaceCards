...
### 2026-09-16 (189차) — 인수인계 문서 갱신 + 검사 스크립트를 저장소로
사용자 요청 "일단 중단하고 지금까지의 작업을 다른 코드로 넘기려 한다.
요약하여 다른 코드에서 이해할 수 있도록 정리해라".
#### Changed
- `00_인수인계_현재상태.md`: **#43 병합과 Naver 지도 버그를 반영해 갱신.**
  새 문서를 만들지 않고 기존 것을 고침 — 이 문서 자체가 "낡은 가이드가 새
  세션을 오도한다"는 문제로 만들어진 것인데, 옆에 두 번째 인수인계 문서를
  두면 같은 문제를 그대로 재현하게 됨.
  - 맨 위에 **지금 깨져 있는 것(Naver 지도)**을 명시하고 §7을 새로 씀 —
    스크린샷으로 배제된 원인(인증·네트워크·404는 전부 아님)과 남은 원인
    (주입 실패), `evaluateJavaScript`에 완료 핸들러가 없어 JS 오류가
    통째로 삼켜진다는 점, 그리고 **다른 프로젝트의 페이지에 의존하는 것이
    근본 문제**라는 것까지.
  - §4.1에 #43이 내린 결정과 **그 이유**를 남김 — 사용량 카운터가 성공한
    요청만 세는 이유, iCloud 기본값을 켜짐으로 둔 이유, 방침을 링크가 아니라
    전문으로 실은 이유, 설정 순서의 근거. 다음 세션이 뒤집기 전에 읽으라고.
  - 규칙 ⑤에 **앱 언어가 iOS를 따른다**는 점과 `LegalDocuments`가 일부러
    번역 사전을 거치지 않는다는 예외를 추가함.
  - 작업 방식에 이번에 실제로 밟은 것들을 추가: **PR 없이는 빌드 검증이 없다**,
    squash 병합 때문에 다음 세션 브랜치가 충돌한다, 브랜치 삭제가 403으로
    막힌다, **새 Swift 파일은 `project.pbxproj` 네 곳에 직접 등록해야 한다**.
#### Added
- `Tools/checks/`(신규): 괄호 균형·번역 MISSING/UNUSED 검사 스크립트와
  기준선 문서. **지금까지 세션마다 새로 쓰고 버렸음** — 매번 다시 쓰는 비용도
  문제지만, 더 나쁜 건 기준선(오탐 2건, MISSING 5/UNUSED 6)을 매번 다시
  측정해야 해서 "원래 있던 실패"와 "내가 만든 실패"를 구분하는 데 시간이
  드는 것이었음.
