...
- **썸네일의 핀 상한은 20개**로 확인됨(81개짜리 실제 목록으로 측정).
  구글이 알려주는 "N places" 개수를 체크섬으로 사용해, 일부만 가져온
  경우 조용히 넘어가지 않고 "N개 중 M개만 가져왔다"고 알리고 캡처
  추가를 안내함. 전체를 가져오려면 정적 메타데이터가 아닌 다른 경로가
  필요함(내부 RPC는 문서화되지 않은 protobuf 파라미터를 요구하고,
