...
| `invalid escape sequence in literal` | 번역 값에 `\—`처럼 Swift가 모르는 이스케이프가 들어감(사전을 스크립트로 고치면서 백슬래시를 겹쳐 씀). Swift는 `\u{2014}`를 원한다. **괄호 균형도 사전 구조도 멀쩡해서 두 번 냈다** → 같은 `check.py`가 이제 이것도 본다 |
...
CI가 **초록인데도 실기기에서만 드러나는** 종류는 또 따로 있다(제스처가 탭을
삼킴, `NavigationSplitView`의 칸이 `NavigationStack`이 아님, `.frame(maxWidth:
.infinity)`에 줄이는 힘이 없음 등). `00_UI개편_기초.md` §5에 실제로 밟은 것들이
모여 있다 — 화면을 건드리기 전에 볼 것.
...
- **코드가 바뀌면 빌드 번호를 올린다** — `project.pbxproj`의
  `CURRENT_PROJECT_VERSION` 네 줄. 문서만 고쳤으면 안 올려도 된다.

  안 올리면 **같은 빌드 번호가 두 가지 코드를 가리킨다.** 실기기에서 "고쳤다는
  버그가 그대로다"라는 보고가 왔을 때, 고친 게 안 먹은 것인지 낡은 빌드인지
  가릴 방법이 없어진다 — 화면 맨 아래 크레딧 줄(`AppCredit`)에 찍히는 그
  번호가 사용자와 나눌 수 있는 유일한 신원이다.

  실제로 그랬다: PR #112가 `MKErrorDomain error 4`를 고치면서 빌드를 안 올려
  **빌드 46이 고치기 전과 고친 뒤 둘 다**가 됐고, 며칠 뒤 같은 오류 스크린샷을
  받고서 코드를 다시 뒤져야 했다.
- 테스트 타깃이 없다. `Tools/` 아래 검사들은 CI가 돌리지 않으니 관련
  ...
  - `Tools/localization/check.py` — 번역 사전. **`Localization.swift`를
    고쳤으면 반드시 돌린다.** 셋을 함께 본다 — 사전 구조가 성한지, 번역이
    빠지지 않았는지, 그리고 값 안의 백슬래시가 Swift가 아는 이스케이프인지
    (위 표의 다섯째·여섯째 사례).
...
- 문서:
  - `CHANGELOG.md` — 어떤 변경이 왜 그렇게 됐는지. **257차(빌드 50, 백업
    메모리 크래시 수정 3단계)까지 최신이다.** 가장 믿을 만하다.
  - `00_UI개편_기초.md` — **화면을 건드린다면 먼저 읽는다.** 지금의 화면
    지도, *갈아엎어도 살아남아야 하는 동작* 목록, 그리고 이 코드베이스에서
    실제로 밟은 SwiftUI 함정들. 이름은 "개편"이지만 개편은 끝났고 지금
    상태를 적고 있다. §6에 **다른 문서들이 얼마나 낡았는지**가 표로 있다.
  - `00_TestFlight_배포.md` — **배포한다면 먼저 읽는다.** 저장소 쪽 준비는
    끝나 있고(`PrivacyInfo.xcprivacy` — 이게 없으면 업로드가 거부된다, 수출
    규정 키, 아이콘), 남은 것은 Xcode와 App Store Connect에서 사람이 하는
    일이다. §1의 **Team ID 불일치**를 먼저 풀어야 한다. §9에 이 환경에서
    확인하지 못한 것이 따로 있다 — 아카이브는 CI가 만들지 않는다.
  ...
  - `00_인수인계_현재상태.md` — 2026-09-23(254차)까지 갱신함. §4·§6이
